    ...
    <div class="w-full sm:w-1/3 p-3">
      <div class="p-6 h-full bg-white border rounded-lg">
        ...
        <h3 class="font-heading mb-3 text-lg font-semibold">
          <a href={"/lifegraph/" + encodeURIComponent(card.label)}> {card.label} ({card.type})</a>
        </h3>
        <p class="text-neutral-500">{card.properties.description || '..'}</p>
        <small class="text-neutral-700">{card.properties.sentiment} {card.properties.tags}</small>

        <small class="text-neutral-700">{new Date(card.last_updated).toDateString()}</small>
      </div>
    </div>
  )
} 